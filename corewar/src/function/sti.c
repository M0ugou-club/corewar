...
int sti_values(process_t *process, vm_t *vm, char *cb_tab)
{
    ...
    reg = get_value(vm->memory, process->index + SKIP_COMM_CB, cb_tab[INDEX_1ST]);
    if (get_reg_error(reg) == -1)
        return (-1);
    val1 = get_value(vm->memory, process->index + SKIP_COMM_CB + 1, cb_tab[INDEX_2ND]);
    val2 = get_value(vm->memory, process->index + SKIP_COMM_CB + 2, cb_tab[INDEX_3RD]);
